...
export default function CraftsmanshipPage() {
  ...
  const [items, setItems] = useState<CraftsmanshipItem[]>([])
  ...
  const fallbackItems: CraftsmanshipItem[] = [
    {
      id: '1',
  ...
  useEffect(() => {
    fetchCraftsmanshipItems()
  }, [])

  const fetchCraftsmanshipItems = async () => {
    try {
      ...
      console.error('Error fetching craftsmanship items:', error)
  ...
  const filteredItems = items.filter(item => {
    ...
    const matchesCategory = item.product_category?.toLowerCase().includes(query)
    const matchesStone = item.stone_type?.toLowerCase().includes(query)
    const matchesLocation = item.site_location?.toLowerCase().includes(query)
    
    return matchesCategory || matchesStone || matchesLocation
  })
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            ...
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-serif font-bold mb-6">
              Our <span className="text-gold">Craftsmanship</span>
            </h1>
              ...
              {filteredItems.map((item, index) => (
                ...
                >
                  ...
                  <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
                    <img
                      src={item.image_url}
                      alt={`${item.product_category || 'Work'} - ${item.site_location || 'Project'}`}
          ...
          <div className="relative w-full max-w-6xl" onClick={(e) => e.stopPropagation()}>
            {selectedMedia.video_url ? (
              ...
              <img
                ...
                alt={`${selectedMedia.product_category || 'Work'} - ${selectedMedia.site_location || 'Project'}`}
            ...
            <div className="mt-4 text-white text-center space-y-2">
              ...
              <div className="flex items-center justify-center gap-4 text-gray-300">
                {selectedMedia.stone_type && (
                  <span className="text-lg">🪨 {selectedMedia.stone_type}</span>
                )}
                {selectedMedia.site_location && (
                  <span className="text-lg">📍 {selectedMedia.site_location}</span>
                )}
              </div>
            </div>
          </div>
